#!/usr/bin/env python3
"""Independent linear 3-D frame screening model for ASTERION FCTA-1 V0.5.
...
def load_model(root: Path):
    model_dir = root / "analysis/ansys/v0_3/model"
    nodes = pd.read_csv(model_dir / "beam_nodes.csv")
    elements = pd.read_csv(model_dir / "beam_elements.csv")
    sections = pd.read_csv(model_dir / "beam_sections.csv").set_index("section_id")
...
def main() -> int:
    ...
    output = (args.output or (root / "calculations/v0_5/python_screening")).resolve()
    output.mkdir(parents=True, exist_ok=True)

    nodes, elements, sections, material, coords, id_to_index, K, mass_diag, cache = assemble(root)
    summaries = []
    displacement_rows = []
    for case in [f"LC-STR-{i:02d}" for i in range(1, 8)]:
